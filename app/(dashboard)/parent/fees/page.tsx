...
export default function ParentFeesPage() {
    ...
    async function loadData() {
      try {
        ...
        if (childList.length > 0) {
          ...
          setSelectedChild({
            id: "s1",
            users: { name: "Sarah Mercer", email: "sarah@school.com" },
            phone: "+2519110000",
            classes: { name: "Grade 9", section: "B" }
          })
        }
      } catch (error) {
        console.error("Error loading parent layout:", error)
      } finally {
        setLoading(false)
      }
    }
  ...
  useEffect(() => {
    ...
    async function loadChildFees() {
      try {
        ...
        console.error("Error loading child fees:", err)
      }
    }
    loadChildFees()
  }, [selectedChild])
  ...
  useEffect(() => {
    if (paymentSuccess === "true" && paidInvoiceId) {
      setAlertMsg({
        type: "success",
        message: "Payment successfully verified! Your child's receipt is ready to download."
      })
    }
  }, [searchParams])
  ...
  const handlePay = async (invoice: any) => {
    ...
    try {
      const payload = {
        ...
        email: selectedChild?.users?.email || "student@school.com",
        name: selectedChild?.users?.name || "Student Name",
        phone: selectedChild?.phone || "+2519000000",
        student_name: selectedChild?.users?.name,
        role: "parent"
      }
      ...
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-gray-500 text-sm">Review child balance reports and clear pending invoices online</p>
          </div>

          {children.length > 1 && selectedChild && (
            <div className="flex items-center gap-2 bg-white px-4 py-2 rounded-xl shadow-sm border border-gray-100">
              <Users className="h-4 w-4 text-amber-600" />
              <span className="text-sm text-gray-600">Active Student:</span>
              <select 
        ...
        <Card className="shadow-lg border-0 bg-white">
          <CardHeader>
            <CardTitle className="text-gray-800 text-lg flex items-center gap-2">
              ...
              Invoicing & Statements: {selectedChild?.users?.name}
            </CardTitle>
            <CardDescription>Fee breakdowns, terms and receipt downloads</CardDescription>
          </CardHeader>
