  ...
  useEffect(() => {
    async function loadMessagingContext() {
      try {
        ...
        for (const kid of childList) {
          const classId = kid.class_id
          if (classId) {
            // Homeroom teacher
            ...
            if (classTeacher?.teachers) {
              ...
              if (!list.some(item => item.id === t.users?.id)) {
                list.push({
                  id: t.users?.id,
                  name: t.users?.name || "Homeroom Teacher",
                  role: `${kid.users?.name || "Child"}'s Homeroom (${classTeacher.name})`,
                  email: t.users?.email || ""
                })
              }
            }
            ...
            subjectsList?.forEach((sub: any) => {
              if (sub.teachers && !list.some((item) => item.id === sub.teachers.users?.id)) {
                list.push({
                  ...
                  role: `${sub.name} Course (${kid.users?.name || ""})`,
        ...
        setContacts(list.length > 0 ? list : [
          ...
          { id: "u-t2", name: "Dr. Evelyn Miller", role: "Math Teacher (Alex's Teacher)", email: "evelyn@school.com" }
        ])

      } catch (error) {
        console.error("Error loading parent chat contacts:", error)
      } finally {
        setLoading(false)
      }
    }
      ...
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <div>
          <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight flex items-center gap-2">
            ...
            Teacher Contact Room
          </h1>
          <p className="text-gray-500 text-sm">Send messages to your child's tutors and homeroom advisors</p>
        </div>
